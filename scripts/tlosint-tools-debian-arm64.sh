...
# Adapted from tlosint-tools.sh — targets Debian 13 Trixie on ARM (UTM / pre-built image)
# Usage: sudo ./tlosint-tools-debian-arm64.sh [--validate-only | --no-validate]
set -uo pipefail
export DEBIAN_FRONTEND=noninteractive

ARCH=$(dpkg --print-architecture)   # arm64 on Apple Silicon / UTM
...
enable_contrib_nonfree() {
  ...
  # Trixie uses DEB822 format in sources.list.d/debian.sources — patch that if present,
  # otherwise fall back to writing /etc/apt/sources.list
...
# ---------- SpiderFoot source + venv (guaranteed fallback) ----------
...
# ---------- Tor Browser (torbrowser-launcher removed from Trixie; use pipx fallback) ----------
...
install_docker_and_compose_if_missing() {
  ...
  if docker compose version >/dev/null 2>&1; then
    ...
    local COMPOSE_ARCH; COMPOSE_ARCH="$(uname -m)"   # aarch64 on ARM64
    local tmp; tmp="$(mktemp -d)"
    (
      cd "$tmp" || exit 0
...
echo "Open your browser manually — check /opt/owlculus/docker-compose.yml for the port."
echo
EOF
...
install_tools_from_list() {
  ...
  # SpiderFoot: APT → pipx → source+venv
  ...
  # Shodan init (auto if env var set; otherwise defer)
  maybe_init_shodan
}
